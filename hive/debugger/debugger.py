...
# TODO :
# Debugger class needs to be divided in multiple classes :
...
class Debugger(Registrable):
    def __init__(self, check_type, app_path=None):
        ...
        # there is already a logger in RLHive, so we need to use their logger!!
        self.logger = settings.file_logger(log_fpath, check_type)
        # we need to follow the same config logic as RLHive !!!!
...
# This class should be named "PreCheckDebugger"
class PreCheck:
    ...
    # TODO : Include the following functions in the super class
